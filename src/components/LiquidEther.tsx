...
`;
    ...
    class Viscous extends ShaderPass {
      ...
      update(opts: { viscous: number, iterations: number, dt: number }) {
        let fbo_in, fbo_out = this.props.output;
    ...
    class Poisson extends ShaderPass {
      ...
      update(opts: { iterations: number }) {
        let p_in, p_out = this.props.output;
        for (let i = 0; i < opts.iterations; i++) {
          if (i % 2 === 0) {
    ...
    class Simulation {
      ...
      fboSize: THREE.Vector2 = new THREE.Vector2();
      cellScale: THREE.Vector2 = new THREE.Vector2();
      boundarySpace: THREE.Vector2 = new THREE.Vector2();
      advection!: Advection;
      ...
      resize() {
        this.calcSize();
        for (let key in this.fbos) {
          const fbo = this.fbos[key];
          if (fbo) fbo.setSize(this.fboSize.x, this.fboSize.y);
        }
      }
      update() {
